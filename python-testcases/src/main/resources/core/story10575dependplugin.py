...
class story10575dependPlugin(Plugin):
    """
    LITP story10575depend plugin
    """
...
#                              ))
    ...
    def _create_depend(self, node_item, model_item, query_dependency):
        ...
        if model_item.is_initial() or model_item.is_updated():
            kwargs = {"path": "/etc/story10575depend{0}.txt".format(
                      model_item.name),
                      "ensure": "present",
    ...
    def create_configuration(self, plugin_api_context):
        ...
        *Example CLI for this plugin:*
        ...
        for node in nodes:
            if self._config_items(node, "depend-story-10575"):
                _sw = node.query("depend-story-10575")[0]
                tasks.extend(
                    self._create_depend(node, _sw, "story-10575a")
                )
