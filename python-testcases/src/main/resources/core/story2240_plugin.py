...
class Story2240Plugin(Plugin):
    ...
    def create_configuration(self, api):
        ms = api.query("ms")[0]
        cfgs = ms.query('story2240-node-config', is_initial=True)
        tasks = [None] * 4
        for cfg in cfgs:
        ...
        #LITPCDS-12781 Config task with persist flag
        cfgs_allstates = ms.query('story2240-node-config')
        task_non_persist = None
        for cfg in cfgs_allstates:
            if cfg.item_id.endswith("_persist"):
                task_non_persist = ConfigTask(ms, cfg,
                        "Config task with persist flag set to False",
                        "notify", "foo")
                task_non_persist.persist = False
        ...
        # Add Task with non persist flag if applicable
        if task_non_persist:
            tasks.append(task_non_persist)
