...
public class LITPcoreTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPcoreTestRunner.class);
    ...
    @TestId(id = "CXP9031108-1", title = "Upgrade ERIClitpcore rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpcoreRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("core");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031108-2", title = "Run python test cases for ERIClitpcore package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpcoreTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpcoreTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031108-3", title = "Parse xml outputs from python tests")
